...
## data types
### native data types
Every value in Python has a class, which determines what type of value it is. Values that share a class also share behavior.  
native data types have two properties. Python includes three native numeric types: integers (`int`), real numbers (`float`), and complex numbers (`complex`).  

differences between `int` and `float` objects: all `int` objects represent integers exactly, while some `float` objects represent a truncated finite approximation to real values.
```
>>> 1 / 3
0.3333333333333333
...
>>> 1 / 3 * 7 * 3   
6.999999999999999  # above two should get the same values
